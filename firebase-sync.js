...
function parseWorkCodeNumber(value){
  const match = String(value || '').toUpperCase().match(/TST-(\d+)/);
  return match ? Number(match[1]) || 0 : 0;
}
function recomputeLastTrabIdFromData(){
  const maxFromData = trabalhos.reduce((max, item) => {
    const num = parseWorkCodeNumber(item?.workCode);
    return num > max ? num : max;
  }, 0);
  lastTrabId = Math.max(Number(lastTrabId || 0), maxFromData);
  return lastTrabId;
}
function nextTrabalhoIdPreview(){
  recomputeLastTrabIdFromData();
  return `TST-${String((Number(lastTrabId || 0) + 1)).padStart(5, '0')}`;
}
function nextTrabalhoId(){
  recomputeLastTrabIdFromData();
  lastTrabId = Number(lastTrabId || 0) + 1;
  try{ localStorage.setItem(STORAGE_KEYS.lastTrabId, String(lastTrabId)); }catch(e){}
  return `TST-${String(lastTrabId).padStart(5, '0')}`;
}
function updateWorkCodePreview(){
  const field = $('workCodeDisplay');
  if(!field) return;
  const editingId = $('trabalhoId')?.value;
  if(editingId){
    const item = trabalhos.find(x => x.id === editingId);
    field.value = item?.workCode || field.value || nextTrabalhoIdPreview();
    return;
  }
